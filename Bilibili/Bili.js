    ...
    switch (true){
      ...
      case /^https?:\/\/app\.bilibili\.com\/x\/v2\/account\/mine/.test(magicJS.request.url):
        try{
          ...
          let obj = JSON.parse(magicJS.response.body);
          let items1 = obj['data']['sections_v2'][1]['items'].filter((e) =>{return item0List.has(e.title);});
          obj['data']['sections_v2'][1]['items'] = items1;
          // 创作中心
          let items2 = obj['data']['sections_v2'][2]['items'].filter((e) =>{return item1List.has(e.title);});
          obj['data']['sections_v2'][2]['items'] = items2;
          // 推荐服务
          let items3 = obj['data']['sections_v2'][3]['items'].filter((e) =>{return item2List.has(e.title);});
          obj['data']['sections_v2'][3]['items'] = items3;
          // 更多服务，去掉课堂模式和青少年模式
          let items4 = obj['data']['sections_v2'][4]['items'].filter((e) =>{return item3List.has(e.title);});
